...
- **Original Three-Globe starfield:** `night-sky.png` restores the original
  visual background from Three-Globe 2.45.2. It is vendored locally with its
  exact versioned URL, SHA-256 digest, size, and 4096×2048 dimensions pinned in
  the manifest; it is decorative context, not astronomical evidence.
- **three-globe 2.45.2 image files:** `night-sky.png`,
  `earth-blue-marble.jpg`, and `earth-topology.png` retain the exact versioned
  URLs, SHA-256 digests, sizes, and dimensions in the manifest.
- **Rights boundary:** Package inclusion establishes byte provenance, not production image-rights clearance.
  NASA/source and package provenance are explicit, but human
