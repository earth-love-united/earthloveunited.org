...
The card leads with two aligned 0–100% electricity-generation bars: one for Ember's published clean aggregate and one for its published fossil aggregate. The bars preserve the source's nine standardized fuel rows—Bioenergy, Coal, Gas, Hydro, Nuclear, Other Fossil, Other Renewables, Solar, and Wind—as visually distinct segments. Nuclear, solar, coal, and the two `Other` buckets use plain fills; hydro, wind, bioenergy, and gas retain restrained static textures. Fixed fuel order and the two-column text/value legend provide the non-color reading route, while hatching is reserved for explicit gaps. The card also retains the published wind-and-solar aggregate as supporting evidence, plus 2019–2024 clean-share change, carbon intensity, and power-sector emissions. Only source-classified annual actuals may enter the comparison order. Source-labelled estimates remain context or gaps.
