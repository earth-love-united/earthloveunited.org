...
The country card may also render a deterministic five-draw in-house Monte Carlo illustration for SSP2-4.5 temperature change. It samples a piecewise-linear quantile model anchored to the published p10, median, and p90 and is truncated to p10–p90. The country ID plus the factual release checksum form the reproducibility seed. Colored paths are only visual bridges from the 1995–2014 baseline reference to sampled 2040–2059 mean changes: they are not CMIP6 model runs, annual forecasts, new evidence, derived metric records, or ranking inputs. The published median and p10–p90 remain the authoritative projection.
...
At-a-glance facts and charted facts are removed from the expanded lens grid so a metric appears only once. Physical records are grouped by variable: observed temperature analysis, the disclosed future-projection illustration, and the unchanged projected-temperature fact are followed by the projected-precipitation fact and observed precipitation data. Separate temperature and precipitation charts show the annual 1970–2025 ERA5 series as solid lines and the compiled OLS fits as dashed lines. Their tables expose every annual value to keyboard and assistive-technology users; the illustrative ensemble exposes its five quantiles and sampled changes in a separate table.
