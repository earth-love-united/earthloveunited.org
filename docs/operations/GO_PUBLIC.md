...
mismatch, and stages only browser files in `_deploy`. First deploy takes about
30 seconds. Subsequent commits to `main` auto-deploy.
...
The globe should render. When WebGL or the library is unavailable, the tested
accessible factual fallback should appear instead. If the generated vendor URL
returns 404, inspect the Cloudflare build log and confirm that the external
project settings exactly use `./tools/build-deploy.sh` and `_deploy`. Do not
bypass the verified local path with a runtime CDN URL, and do not commit the
generated 1.8 MB file.
