...
function validateInputs(input) {
  ...
  assert(dataReview.reviewed_commit_sha === uiReview.reviewed_commit, 'data and UI reviews do not pin the same CT-42 builder head');

  for (const [path, expected] of Object.entries(dataReview.reviewed_input_sha256 || {})) {
    assert(fileHashes[path] === expected, `CT-42 data review pin mismatch: ${path}`);
  }
  for (const [path, expected] of Object.entries(pinMap(uiReview.reviewed_file_pins || []))) {
...
function buildGateCandidate(input, source) {
  ...
  return {
    ...
    review_context: {
      ...
      reviewed_builder_commit: input.dataReview.reviewed_commit_sha,
...
function compile(input) {
  const source = validateInputs(input);
  const gateCandidate = buildGateCandidate(input, source);
  const inputArtifact = {
    schema_version: '1.0.0',
    adapter_version: ADAPTER_VERSION,
...
module.exports = { ADAPTER_VERSION, EVALUATED_AT, METHODOLOGY_VERSION, REQUIRED_REASON_CODES, compile, hash, validateInputs };
