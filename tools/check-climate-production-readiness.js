...
const report = evaluateReadiness({
  ...
  ct40: {
    decision_scope: ct40.decision_scope,
    decision: ct40.decision,
    eligible: ct40.eligible ?? ct40.release_eligible,
    content_eligible: reviewedRelease.content_eligible === true,
    factual_publication_eligible: ct40.publication_tiers?.factual_display?.eligible === true,
    magnitude_comparison_eligible: ct40.publication_tiers?.magnitude_comparison?.eligible === true,
    commitment_display_status: ct40.publication_tiers?.commitment_display?.status || 'not_present',
    derived_metrics_status: ct40.publication_tiers?.derived_metrics?.status || 'not_present',
    performance_assessment_status: ct40.publication_tiers?.performance_assessment?.status || 'not_present',
    score_status: ct40.publication_tiers?.score?.status || 'not_present',
