...
assert(serviceWorker.includes("const CACHE_NAME = 'elu-v50-era5-precipitation'"), 'service-worker cache epoch is not v50');
assert(index.includes("navigator.serviceWorker.register('/sw.js?v=50-era5-precipitation'"), 'HTML does not request the v50 service worker');
for (const asset of [
...
assert(dataSource.includes("version: 'cci1candidate5'"), 'Data runtime query does not match service-worker staging');
...
console.log(`Country Climate Intelligence atomic staging check passed (elu-v50; runtime ${fileSha256(runtimePath)}; rollback retained).`);
