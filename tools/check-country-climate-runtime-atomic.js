...
assert(serviceWorker.includes("const CACHE_NAME = 'elu-v60-power-title-cleanup'"), 'service-worker cache epoch is not v60');
assert(index.includes("navigator.serviceWorker.register('/sw.js?v=60-power-title-cleanup'"), 'HTML does not request the v60 service worker');
for (const asset of [
...
console.log(`Country Climate Intelligence atomic staging check passed (elu-v60; runtime ${fileSha256(runtimePath)}; rollback retained).`);
