...
assert(serviceWorker.includes("const CACHE_NAME = 'elu-v45-country-climate-fair-hud'"), 'service-worker cache epoch is not v45');
assert(index.includes("navigator.serviceWorker.register('/sw.js?v=45-country-climate-fair-hud'"), 'HTML does not request the v45 service worker');
for (const asset of [
...
assert(dataSource.includes("version: 'cci1candidate2'"), 'Data runtime query does not match service-worker staging');
...
console.log(`Country Climate Intelligence atomic staging check passed (elu-v45; runtime ${fileSha256(runtimePath)}; rollback retained).`);
