...
assert(serviceWorker.includes("const CACHE_NAME = 'elu-v77-cci-raw-byte-boundary'"), 'service-worker cache epoch is not v77');
assert(index.includes("navigator.serviceWorker.register('/sw.js?v=77-cci-raw-byte-boundary'"), 'HTML does not request the v77 service worker');
for (const asset of [
...
assert(dataSource.includes("version: 'cci1runtime13'"), 'Data runtime query does not match service-worker staging');
...
console.log(`Country Climate Intelligence atomic staging check passed (elu-v77; runtime ${fileSha256(runtimePath)}; rollback retained).`);
